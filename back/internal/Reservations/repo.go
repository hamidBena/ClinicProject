...
func (r *Repository) Create(reservation *Reservation) error {
	...
	result, err := r.db.Exec(
		`INSERT INTO reservations (timeCreated, queue_id, account_id, status)
		 VALUES (?, ?, ?, ?)`,
		timeCreated,
		reservation.QueueID,
...
func (r *Repository) GetByID(id int64) (*Reservation, error) {
	...
	var timeCreated, status string
	err := r.db.QueryRow(
		`SELECT id_reservation, timeCreated, queue_id, account_id, status
		 FROM reservations
...
func (r *Repository) ListByAccountID(accountID int64) ([]Reservation, error) {
	...
	rows, err := r.db.Query(
		`SELECT r.id_reservation, r.timeCreated, r.queue_id, r.account_id, r.status,
				COALESCE(s.name, ''),
...
func (r *Repository) ListByQueueID(queueID int64) ([]Reservation, error) {
	...
	for rows.Next() {
		var res Reservation
		var timeCreated, status, patientName string
		if err := rows.Scan(
			&res.ID,
			&timeCreated,
			&res.QueueID,
